...
/// One upserted preview-deploy bot comment for a pull request.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PullPreviewComment {
    /// Owning pull request id. Unique; the preview bot never appends duplicate rows.
    pub pull_id: String,
    /// SiteFlow build job id returned by the preview trigger.
    pub build_job_id: String,
    /// Raw deployment status from SiteFlow (`building`, `ready`, `failed`, ...).
    pub deployment_status: String,
    /// Per-deployment preview URL. Empty until SiteFlow reports one.
    pub preview_url: String,
    /// Full commit SHA submitted for the preview deploy.
    pub commit_sha: String,
    /// Plain bot body generated by Loom for display/audit.
    pub body: String,
